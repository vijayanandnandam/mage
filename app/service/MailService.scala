...
class MailService @Inject()(implicit mailerClient: MailerClient, configuration: play.api.Configuration) {
  ...
  def sendMail(to: String, subject: String, bodyText: Option[String] = None,
    ...
    val finalReplyTo: Option[String] = Some(replyTo.getOrElse(configuration.underlying.getString("mail.default.reply.to")))
    ...
    logger.debug(" reply to >>>  " + finalReplyTo.get)
    logger.debug(" bounce to >>>  " + bounceAddress.get)
