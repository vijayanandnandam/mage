...
@Singleton
class MongoDbService @Inject()(implicit val ec: ExecutionContext, reactiveMongoApi: ReactiveMongoApi, configuration: play.api.Configuration) extends DBConstants{
  ...
  def userDocument(username : Option[String], userid : Option[Long], folio : Option[Long], firstName : Option[String],
                   pan : Option[String], mob : Option[String],signingKey :  Option[String], isVerified: Option[String]= None): (BSONDocument,JsValue,String) = {
    ...
    val _isVerified = isVerified.getOrElse(N_FLAG)
    val userObject = new UserLoginObject(username, userid, folio, firstName, pan, mob, Some(_isVerified))
