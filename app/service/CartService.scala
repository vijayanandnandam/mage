...
class CartService @Inject()(implicit val ec: ExecutionContext, mongoDbService: MongoDbService, schemeService: SchemeService) {
      ...
      coll.find(userCartSelector).options(QueryOpts().batchSize(10)).cursor[BSONDocument]().collect[List](10, Cursor.FailOnError[List[BSONDocument]]()).flatMap(listOfDocuments => {
        if(listOfDocuments.nonEmpty){
          ...
          listOfDocuments.foreach(bsonDoc  =>{
            val cartObject = (BSONFormats.BSONDocumentFormat.writes(bsonDoc).as[JsObject])
            if (cartObject.keys.contains("funds")){
          ...
          coll.find(idCartSelector).one[BSONDocument].flatMap(nonLoginFunds => {
            ...
            var allNonLoginJsFunds: List[JsObject] = List[JsObject]()
            val cartObject = (BSONFormats.BSONDocumentFormat.writes(nonLoginFunds.get).as[JsObject])
            if (cartObject.keys.contains("funds")) {
              allNonLoginJsFunds = (BSONFormats.BSONDocumentFormat.writes(nonLoginFunds.get).as[JsObject] \ "funds").as[List[JsObject]];
            }
          ...
          coll.find(idCartSelector).one[BSONDocument].flatMap(nonLoginFunds => {
            ...
            var nonLoginfJsList: List[JsObject] = List[JsObject]()
            val cartObject = (BSONFormats.BSONDocumentFormat.writes(nonLoginFunds.get).as[JsObject])
            if (cartObject.keys.contains("funds")) {
              nonLoginfJsList = (BSONFormats.BSONDocumentFormat.writes(nonLoginFunds.get).as[JsObject] \ "funds").as[List[JsObject]]
            }
  ...
  def removeFund(coll: BSONCollection, username: String, soptRfnum: String, cartid: String): Future[Option[Long]] = {
    if(cartid.length==0 && soptRfnum.length==0){
      logger.debug("cartid OR soptRfnum not provided cartid [" + cartid+"] sopt ["+soptRfnum+"]");
      return Future.apply(None)
    }

    val loginCartSelector = BSONDocument("_id" -> BSONObjectID.parse(cartid).get)
    coll.find(loginCartSelector).one[BSONDocument].flatMap(funds => {
      if (funds.isEmpty) {
        Future.apply(Some(0L))
      } else {
        ...
        schemeService.getSchemeIdByOptionId(soptRfnum).flatMap(smtRfnum => {
          ...
          mongoDbService.updateDoc(coll, loginCartSelector, cartModifier).map(_ => Some(smtRfnum));
        })
      }
    })
  }
