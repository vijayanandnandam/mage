...
import models.{FundDetails, FundTransaction, RedemptionDetails}
...
class CapitalGainsReportService {
  ...
  def calculateCapitalGains(fundDetailsList: ListBuffer[FundDetails],purchaseFundsList: ListBuffer[FundDetails]): ListBuffer[FundDetails] = {
    ...
    for (currFund <- fundDetailsList) {
      ...
      fundDetailsList(fundIndex) = fund.copy(redemptionDetailsList = Some(redemptionDetailsList), totalRedeemedUnits = Some(totalUnitsRedeemed), totalUnitsLeft = Some(totalUnitsLeft), totalAmount = None, totalShortTermGain = totalShortTermGain, totalLongTermGain = totalLongTermGain)
