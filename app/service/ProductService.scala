...
import data.model.Tables.{FcamctRow, FcbseRow, FcctmtRow, FcdrdRow, FcdsdRow, FcramtRow, FcrpaRow, FcsmtRow, FcsoelRow, FcsoptRow}
...
class ProductService @Inject()(productRepository: ProductRepository, schemeRepository: SchemeRepository,
    ...
    product.flatMap(product => {
      productRepository.getProductStateObjectList(product.id).flatMap(productStateObjectList => {
        val futures = for (productStateObject <- productStateObjectList) yield {
          ...
          var defaultSoptRfnum: Option[Long] = None;
          ...
          soptRfnumArray.zipWithIndex.foreach((data) => {
            ...
            if (soptRfnumArray.length == 1 || isDefault == Y_FLAG) {
              defaultSoptRfnum = Some(soptRfnum)
            }
          ...
          getFundObjBySchemeId(new Product(productStateObject.productRfnum.get,
            productStateObject.productName.get, productStateObject.smtRfnum.get, productOptions.toList), defaultSoptRfnum)
        }
  ...
  def getFundObjBySchemeId(product: Product, defaultSoptRfnum: Option[Long] = None): Future[Option[FundDoc]] = {

    var fundDocObj = solrFundSearchService.fundSearchBySmtId(product.smtRfnum).funds.head

    fundDocObj = fundDocObj.copy(productObj = Some(product), selected = Some(true), defaultSopt = defaultSoptRfnum)
    Future.apply(Some(fundDocObj))
    /*val queries = for {
      ...
      rpaObj <- schemeRepository.getRPARowBySchemeOption(soptObj.id)
      bseObj <- schemeRepository.getBSERowBySchemeOption(soptObj.id)
    ...
    } yield (smtObj, soptObj, scatObj, catObj, amcObj, dsdObj, drdObjList, ratioObj, sipAmount, exitLoadObj,rpaObj,bseObj)

    queries.map(values => {
        Some(getFundObj(values._1, values._2, values._3, values._4, values._5, values._6, values._7, values._8, values._9, values._10, product, values._11, values._12))
    })*/
  }
  ...
  def getFundObj(smtObj: FcsmtRow, soptObj: FcsoptRow, scatObj: FcctmtRow, catObj: FcctmtRow, amcObj: FcamctRow, dsdObj: Option[FcdsdRow],
                 drdObjList: Seq[FcdrdRow], ratioObj: Option[FcramtRow], sipAmount: Option[Int], soelObj: Option[FcsoelRow], productObj: Product,
                 rpaObj: FcrpaRow, bseObj: FcbseRow): FundDoc = {
    ...
    var fundBasicFactSheet = Json.obj(
      ...
      "exitLoad" -> exitLoad,
      "benchmarkName" -> "TBD",
      "minLumpsum" -> schemeHelper.getMinLumpSumAmount(rpaObj, bseObj),
