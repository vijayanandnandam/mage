...
class PortfolioSummaryService {
  ...
  val logger, log = LoggerFactory.getLogger(classOf[PortfolioSummaryService])
  ...
  def calculateCostValue(fundReportList: ListBuffer[FundDetails]): AssetClassCostValueModel = {
    ...
    for (fund <- fundReportList) {
      ...
      for (transaction <- purchaseTransactions) {
        if(redeemTransactions.size == 0){
          fundCostValue += transaction.amount.getOrElse(0.0)
        } else{

          fundCostValue += (transaction.units * transaction.price)//transaction.amount.getOrElse(0.0)
        }
      }
