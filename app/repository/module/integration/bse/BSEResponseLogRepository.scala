...
@Singleton
class BSEResponseLogRepository @Inject()(integrationRepository: IntegrationRepository) extends IntegrationConstants {
  ...
  private def saveResponseLog(paramListTuple: (ListBuffer[String], ListBuffer[String]), integrationId: String, userName: String) = {
    ...
    integrationRepository.saveResponseParameters(BSE_INTEGRATION_NAME, Some(integrationId), paramNameList, paramValueList, userName)
  }
