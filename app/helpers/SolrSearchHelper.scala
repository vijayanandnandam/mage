...
class SolrSearchHelper @Inject()(config: Configuration,
  ...
  val folioSolrClient = new HttpSolrClient.Builder(config.getString("solr.url") + config.getString("solr.core." + FOLIO_CORE)).withHttpClient(solrPoolingClientConnectionManager.getHttpClient).build()
  ...
  def execute(query: SolrQuery, coreName: String): QueryResponse = {
    ...
    } else if (coreName == FUND_CORE) {
      fundSolrClient.query(query)
    } else if (coreName == FOLIO_CORE) {
      folioSolrClient.query(query)
    } else {
      logger.debug("NO such core Configured")
      new QueryResponse()
    }
  }
