...
@Singleton
class ZendeskHelper @Inject()(implicit val ec: ExecutionContext, configuration: play.api.Configuration, lifeCycle: ApplicationLifecycle) extends CNDConstants with ZendeskConstants{
  ...
  logger.info("Initializing Zendesk ASYNC_HTTP_CLIENT")
  ...
  lifeCycle.addStopHook(() => {
    Future.successful(
      if(!ASYNC_HTTP_CLIENT.isClosed) {
      logger.info("Closing Zendesk ASYNC_HTTP_CLIENT")
  ...
  def getUserIdByEmail(email:String):Long = {
    ...
    if (email != null && email.length!=0) {
      ...
      if(userId.equals(0L)){
        val user = new org.zendesk.client.v2.model.User(true, email, email)
        val userRetval = zendesk.createUser(user)
        userId = userRetval.getId
      }
    }
    userId
  }
