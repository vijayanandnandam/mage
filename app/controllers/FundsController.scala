...
import service.{SchemeService, SolrFundSearchService}

import scala.concurrent.ExecutionContext.Implicits.global


class FundsController @Inject()(solrFundSearchService: SolrFundSearchService, auth: AuthenticatedAction,
                                schemeService: SchemeService) extends Controller {
  ...
  def getFacets = auth.Action(parse.json) { request =>
    ...
    Ok(Json.toJson(new FundSearchFacets(
      ratingFilterData = Some(solrFundSearchService.getRatingList(searchQuery)),
      ageFilterData = Some(solrFundSearchService.getAgeList(searchQuery)),
      aumFilterData = Some(solrFundSearchService.getAUMRange(searchQuery)),
      amcFilterData = Some(solrFundSearchService.getAmcList(searchQuery)),
      categoryFilterData = Some(solrFundSearchService.getCategoryList(searchQuery)),
      subCategoryFilterData = Some(solrFundSearchService.getSubCategoryList(searchQuery))
    )));
  ...
  def getFundBySoptRfnum = auth.Action.async(parse.json) { request =>

    var soptRfnum = (request.body \ "soptrfnum").as[Long];
    schemeService.getMergedSoptForPurchase(soptRfnum).flatMap(mergedSoptId =>{
      if(mergedSoptId.nonEmpty){
        soptRfnum = mergedSoptId.get
      }
      schemeService.getSchemeIdByOptionId(soptRfnum.toString).map(smtrfnum => {
        Ok(Json.toJson(solrFundSearchService.fundSearchBySmtId(smtrfnum).funds))
      })
    })
  }

  def checkPurchaseAllowed = auth.Action.async(parse.json){request =>
    var soptRfnum = (request.body \ "soptrfnum").as[Long];
    schemeService.getMergedSoptForPurchase(soptRfnum).flatMap(mergedSoptId => {
      if (mergedSoptId.nonEmpty) {
        soptRfnum = mergedSoptId.get
      }
      schemeService.isPurchaseAllowed(soptRfnum).map(purchaseAllowed =>{
        Ok(Json.obj("PurchaseAllowed" -> purchaseAllowed))
      })
    })
  }
