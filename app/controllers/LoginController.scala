  ...
  def getLoginToken = auth.Action.async(parse.json) { request =>
    ...
    userRepository.signInUser(username, password, ipAddress).flatMap(userTuple => {
      if (userTuple._1 != 0 && userTuple._2 && userTuple._3) {
        ...
        userRepository.getUBDandEUBDbyUserid(userid).flatMap(users => {
          if (users.nonEmpty && users.length > 0){
            val user = users.head._1
            val eubdUser = users.head._2
            val retval_tuple = mongoDbService.userDocument(Some(username),Some(userid),Some(folioCount),user.ubdfirstname,user.ubdpan,user.ubdmobileno,None, Some(eubdUser.eubdisvarified))
      ...
      if (userid > 0) {
        userRepository.getUBDandEUBDbyUserid(userid).flatMap(users => {
          if (users.nonEmpty && users.length > 0) {
            val user = users.head._1
            val eubdUser = users.head._2
            val retval_tuple = mongoDbService.userDocument(Some(username),Some(userid),None,user.ubdfirstname,user.ubdpan,user.ubdmobileno,None, Some(eubdUser.eubdisvarified))
  ...
  def socialLogin = auth.Action.async(parse.json){ request => {
    ...
    userRepository.getUserByUsername(email).flatMap(_user => {
      ...
      if (_user.nonEmpty){
        ...
        userRepository.getUBDandEUBDbyUserid(userid).flatMap(users => {
          if (users.nonEmpty && users.length > 0){
            val ubd = users.head._1
            val eubd = users.head._2
            val username = email
            val userTuple = mongoDbService.userDocument(Some(username),Some(userid),Some(folioCount),user.ubdfirstname,user.ubdpan,user.ubdmobileno,None, Some(eubd.eubdisvarified))
            collection(TOKEN_COLLECTION_NAME).flatMap { coll =>
          ...
          if (userid > 0) {
            userRepository.getUBDandEUBDbyUserid(userid).flatMap(users => {
              if (users.nonEmpty && users.length > 0) {
                val user = users.head._1
                val eubdUser = users.head._2
                val userTuple = mongoDbService.userDocument(Some(username),Some(userid),Some(folioCount),user.ubdfirstname,user.ubdpan,user.ubdmobileno,None, Some(eubdUser.eubdisvarified))
                /*sending welcome mail*/
