  ...
  def updateuserKYCStatus(userid: Long, username: String, kycStatus: String, kycType: String): Future[Boolean] = {
    ...
    db.run(query.result).flatMap(values => {
      if (values.isEmpty){
        ...
        // Update in FCKYC
        var query2 = for {
            kycInstance <- Fckyc.filter(x => x.kycubdrfnum === userid).map(x => (x.kycstatus, x.kyctype, x.modifydate)).update((kycStatus, Option(kycType), DateTimeUtils.getCurrentTimeStamp()))
        } yield ()
        db.run(query2).map(values => {
          true
        }).recover {
...
//      var row = value.headOption.get
  ...
  def updateDocumentPath(ipAddress: String, userid: Long, username: String, dmtid: String, path: String, doctypecnd: Long): Future[(String, String, String)] = {
    var fcdmtRowWrapper = new FcdmtRowWrapper(None, doctypecnd, Option(path), None, None, None, None, Option(ipAddress))
    ...
    if (dmtid.isEmpty){
      ...
      // Update in FCKMT
      var query2 = for {
        dmtInstance <- Fcdmt.filter(x => x.id === dmtid.toLong).map(x => (x.dmtcndtyperfnum, x.dmtpath, x.dmtuploadip, x.modifydate)).update((doctypecnd, Option(path), Option(ipAddress), DateTimeUtils.getCurrentTimeStamp()))
      } yield ()
      db.run(query2).map(values => {
