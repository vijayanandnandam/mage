...
class Application @Inject() (implicit ws: WSClient, messagesApi: MessagesApi,auth: AuthenticatedAction) extends Controller{
  ...
  def startGC = auth.Action { request => {
    System.gc()

    Redirect("/server/getJVMUsage");
  }}
}
