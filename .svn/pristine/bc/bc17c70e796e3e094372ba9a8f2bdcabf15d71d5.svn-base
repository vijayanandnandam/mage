...
class PasswordRepository @Inject()(implicit ec: ExecutionContext, protected val dbConfigProvider: DatabaseConfigProvider,
                                   ...
                                   @Named("isLive") isLive: Boolean, bSEUploadResponseHelper : BSEUploadResponseHelper, bSEOrderResponseHelper : BSEOrderResponseHelper)
  ...
  def getBSEEncryptedPassword(bsePassCode: String): Future[(String, Timestamp, String)] = {
    val query = {
  ...
  def updateBSEPassword(): Future[Int] = {
    ...
    integrationRepository.getDefaultParamValues(bseDefaultParamList, BSE_INTEGRATION_NAME).flatMap(parameterMap => {
      ...
      val query = {
        ...
        logger.debug("BSE Order Api Encrypted Password = " + encryptedPassword)
        sqlu"""UPDATE FCPASS SET PASSKEY = ${encryptedPassword}, PASSEXPIRYDATE = NOW() + INTERVAL 280 SECOND,PASSWORDPASSKEY = ${passKey}, MODIFYDATE = CURRENT_TIMESTAMP WHERE PASSCODE = ${BSE_ORDER_API_PASS_CODE} """
        ...
        val bsePasswordResponse = getPasswordResponse(getBSEPassword)
        val encryptedPassword = bsePasswordResponse.getBSEPasswordResponse.encryptedPassword
        logger.debug("BSE Upload Api Encrypted Password = " + encryptedPassword)
        sqlu"""UPDATE FCPASS SET PASSKEY = ${encryptedPassword}, PASSEXPIRYDATE = NOW() + INTERVAL 280 SECOND,PASSWORDPASSKEY = ${passKey}, MODIFYDATE = CURRENT_TIMESTAMP WHERE PASSCODE = ${BSE_UPLOAD_API_PASS_CODE} """
