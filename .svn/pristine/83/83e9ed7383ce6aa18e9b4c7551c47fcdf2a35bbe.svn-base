...
class UserController @Inject()(implicit val ec: ExecutionContext, auth: AuthenticatedAction, userService: UserService,
                ...
                if (istktAllowed) {
                  ...
                  zendeskRepository.getTicketsByUserIdANDpurpose(userid, purpose).flatMap(fctktRows => {
                    ...
                    if (tktOption.nonEmpty) {
                      ...
                      val customFieldRes = zendeskService.setCustomField(tktId, customFieldList, None, Some("SYSTEM COMMENT : PAN UPDATED"), None)
  ...
  def activateUser = auth.Action.async(parse.json) { request =>
    ...
    userService.activateUser(userid, ipAddress).map(responseTuple => {
      if (responseTuple._1) {
        ...
        val updateTickets = zendeskService.updateTicketsOnUserVerification(userid, Some(zendeskHelper.TKT_PURPOSE_DOCUMENT_UPLOADED), Some("SYSTEM COMMENT: User marked as verified (Account Active)")).map(status=>{
          logger.debug("activateUser ["+userid+"] Tickets update status >>> " + status)
          status
        })
  ...
  def registerInBSE = auth.Action.async(parse.json) { request => {
    ...
    userService.bseRegistration(userid, ipAddress).map(value => {
      val ticketComment = "SYSTEM COMMENT: BSE REGISTRATION : "+ value._1+" FATCA : " + value._2
      val updateTickets = zendeskService.updateTicketsOnBseRegistration(userid, Some(zendeskHelper.TKT_PURPOSE_DOCUMENT_UPLOADED), Some(ticketComment)).map(status=>{
        logger.debug("registerInBSE ["+userid+"] Tickets update status >>> " + status)
        status
      })
                            ...
                            if (istktAllowed) {
                              ...
                              zendeskRepository.getTicketsByUserIdANDpurpose(userId, Some(TKT_PURPOSE_REGISTRATION)).flatMap(fctktRows => {
                                ...
                                Future.sequence(for (sendfile <- listofFiles) yield {
                                  ...
                                  zendeskRepository.getTicketsByUserIdANDpurpose(userId, Some(TKT_PURPOSE_DOCUMENT_UPLOADED)).flatMap(fctktRows => {
                                    if (fctktRows.isEmpty) {
                                      zendeskService.createTicket(newRequester, "Documents Uploaded", Some(comment), Some(zendeskHelper.ZENDESK_GROUP_ONBOARDING), userName, userName, userId, None, Some(TKT_PURPOSE_DOCUMENT_UPLOADED), None).map(newTktId => {
      ...
      userService.getUserObjectFromReq(request).flatMap(userLoginObject => {
        ...
        zendeskRepository.getTicketsByUserIdANDpurpose( userId, purpose).flatMap(fctktRows =>{
          ...
          if (tktOption.nonEmpty) {
            val tktId = tktOption.get
            logger.debug("zendesk ticket id >>>> " + tktId)
            val customFieldRes = zendeskService.setCustomField(tktId, customFieldList, None, Some("SYSTEM COMMENT : REDIRECTED FOR EKYC"), None)
            logger.debug("Customfield Status -->" + customFieldRes)
