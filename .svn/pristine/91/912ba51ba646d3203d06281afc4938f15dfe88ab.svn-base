...
class CartController @Inject()(schemeService: SchemeService, auth: AuthenticatedAction, userService: UserService,
  ...
  def saveCart = auth.Action.async(parse.json) { request =>
    ...
    userService.getUsernameFromRequest(request).flatMap { usernameOpt => {
      ...
      collection(CART_COLLECTION_NAME).flatMap { coll =>
        coll.find(findQuery).one[BSONDocument].flatMap(doc => {
          if (doc.isEmpty) {
            ...
            logger.debug("cart id "+ cartId.get.stringify +" found updating funds >>> " + funds.length)
            val writeRes : Future[UpdateWriteResult] = mongoDbService.updateDoc(coll, findQuery, BSONDocument("username" -> username, "funds" -> funds))
            writeRes.onComplete{
              case Failure(e) => {
                logger.error("Mongo Error :: " + e.getMessage + " in saving >>> " + cartId.get.stringify)
                Ok(Json.obj("success" -> false, "error" -> e.getMessage, "message" -> e.getMessage))
  ...
  //    var findQuery = BSONDocument("username" -> username, "_id" -> BSONObjectID.parse(cartId).get)
        ...
        case dex: DatabaseException => {
          Ok(Json.obj("error" -> dex.message,"message" -> dex.message, "success"-> false))
        }
        case e: Throwable => {
          Ok(Json.obj("error" -> e.getMessage,"message" -> e.getMessage, "success"-> false))
        }
      }
    }
