...
class SolrFundSearchService @Inject()(solrSearchHelper: SolrSearchHelper, schemeRepository: SchemeRepository) extends SolrConstants {
  ...
  val logger, log = LoggerFactory.getLogger(classOf[SolrFundSearchService])
  ...
  def getAmcList(searchQuery: SearchQuery): ListBuffer[FundFilter] = {
    ...
    solrQuery = addActivatedFundFilter(solrQuery);
  ...
  def getAgeList(searchQuery: SearchQuery): ListBuffer[FundFilter] = {
    ...
    query = addActivatedFundFilter(query);
  ...
  def getRatingList(searchQuery: SearchQuery): ListBuffer[FundFilter] = {
    ...
    facetList.foreach { (facet: Facet) =>
      ...
      ratingFilterData += new FundFilter(abc(0).slice(1, abc(0).length + 1), abc(0).slice(1, abc(0).length + 1), facet.count, false);
    }
    return ratingFilterData;
  }

  def addActivatedFundFilter(solrQuery: SolrQuery): SolrQuery = {
    solrQuery.add("fq", FUND_SOP_DEF + ":" + ACTIVE)
    solrQuery.add("fq", FUND_SOP_ACT + ":" + ACTIVE)
    solrQuery.add("fq", FUND_SMT_ACT + ":" + ACTIVE)
    solrQuery.add("fq", FUND_AMC_ACT + ":" + ACTIVE)
    solrQuery.add("fq", FUND_BSE_PUR + ":" + ACTIVE)
    solrQuery
  }

  def getAUMRange(searchQuery: SearchQuery): ListBuffer[FundFilter] = {
    var query = new SolrQuery();
    query.setQuery(searchQuery.q.get);
    query = addActivatedFundFilter(query);
    val intervals = Array("[0,100)", "[100,500)", "[500,1000)", "[1000,2000)", "[2000,5000)", "[5000,10000)", "[10000,50000)");
    val facetList = solrSearchHelper.getFacetDataForIntervals(query, "*", FUND_D_AUM, intervals, FUND_CORE);
    val aumFilterData = ListBuffer.empty[FundFilter];
    facetList.foreach { (facet: Facet) =>
      val abc = facet.name.split(',');
      //println(abc(0));
      aumFilterData += new FundFilter(abc(0).slice(1, abc(0).length + 1), abc(0).slice(1, abc(0).length + 1), facet.count, false);
    }
    return aumFilterData;
  }

  /**
    * Prepares and executes Query for fund search on smt id
    *
    * @param smtRfnum
    * @return
    */
  def fundSearchBySmtId(smtRfnum: Long): FundSearchResult = {
    var query = new SolrQuery();
    query.setQuery("*");
    query.add("fq", FUND_ID + ":" + smtRfnum);
    query = addActivatedFundFilter(query);
  ...
  def getFundFromFundsSolrDoc(doc: SolrDocument): FundDoc = {
    ...
    val sip = doc.getFieldValue(FUND_SIP_ACT);
    if (sip != null) {
      if (sip == ACTIVE) {
  ...
  def getFundSearchQuery(searchQuery: SearchQuery): SolrQuery = {
    ...
    if (searchQuery.q.get.isEmpty && searchQuery.se.get.isEmpty) {
      //logger.debug("Inside default filtering");
    ...
    if (searchQuery.ar.isDefined && searchQuery.ar.get.length > 0) {
      var aumFilterString: String = "(";
      val aumRanges = searchQuery.ar.get;
      aumRanges.zipWithIndex.foreach(listTuple => {
        val filterVal = listTuple._1;
        val index = listTuple._2;
        if (index == aumRanges.length - 1) {
          aumFilterString += "[" + filterVal + "])"
        } else {
          aumFilterString += "[" + filterVal + "]" + OR_OPERATOR
        }
      })
      query.add("fq", FUND_D_AUM + ":" + aumFilterString);
    ...
    if (searchQuery.o.isDefined && searchQuery.o.get.length > 0) {
      if (searchQuery.o.get.split(':')(0) == "sip") {
        query.add("fq", FUND_SIP_ACT + ":" + ACTIVE);
      }
    }

    addActivatedFundFilter(query);
  }
