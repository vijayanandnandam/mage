...
class BSEClientService @Inject()(protected val dbConfigProvider: DatabaseConfigProvider, implicit val ec: ExecutionContext, bSEClientHelper: BSEClientHelper,
  ...
  def registerClient(clientCode: Long, userName: String, userBasic: UserBasic, userAddress: UserAddress, userBank: UserBank, userFatca: UserFatca, nominee: Option[Nominee]): Future[Boolean] = {
    ...
    bSEClientHelper.populateClientModel(clientCode, userBasic, userAddress, userBank, userFatca, nominee).flatMap(bSEClientDetailsModel => {
        ...
        userRepository.getUserFatca(userid).flatMap(userFatca => {
          userRepository.getUserBank(userid).flatMap(userBank => {
            userRepository.getUserNominee(userid).flatMap(userNominee => {
              this.registerClient(userid, username.get, userBasic, userAddress, userBank, userFatca, Some(userNominee)).flatMap(response => {
                if (response) {
  ...
  def getBSERegistrationStatus(userid: Long): Future[Boolean] = {
    ...
    fceubdRepo.filter(x => x.id === userid).map(row => {
      if (row.headOption.nonEmpty && row.headOption.get.eubdisexternallyreg.equalsIgnoreCase(Y_FLAG)) {
        status = true
      }
      status
    })
  ...
  def getBSEFatcaStatus(userid: Long): Future[Boolean] = {
    ...
    fceubdRepo.filter(_.id === userid).map(row => {
      if (row.headOption.nonEmpty && row.headOption.get.eubdisexternallyfatca.equalsIgnoreCase(Y_FLAG)) {
        fatcaStatus = true
      }
      fatcaStatus
    })
