...
import constants._
import data.model.Tables.{Fcact, Fcbse, Fcbsuf, FcbsufRow, Fcebse, Fcholi, Fcoht, FcohtRowWrapper, Fcomt, FcomtRow, FcomtRowWrapper, Fcooa, FcooaRowWrapper, Fcosht, FcoshtRow, FcoshtRowWrapper, Fcostm, FcostmRow, Fcotpt, FcotptRow, FcotptRowWrapper, Fcsoct, FcsoctRow, FcsoctRowWrapper, Fcsopt, Fcsot, FcsotRow, FcsotRowWrapper}
...
class OrderRepository @Inject()(implicit ec: ExecutionContext, protected val dbConfigProvider: DatabaseConfigProvider, fcbseRepo: FcbseRepo,
  ...
  def placeNewOrder(orderModel: OrderModel, soptCategoryMap: mutable.HashMap[Long, String], allotmentDateMap: mutable.HashMap[Int, String],
    ...
    checkSchemeFolioNo(orderModel.subOrders, userLoginObject).flatMap(subOrderList => {
      ...
      for (subOrderTuple <- subOrderZipList) {
        ...
        if(subOrder.investmentMode == SIP_INVESTMENT_MODE){
          val sipStartDate = getEstimatedSIPDate(subOrder.sipDayOfMonth.get)
          val fcsoctApproxAllotmentRow = new FcsoctRowWrapper(None, SIP_START_DATE_KEY, sipStartDate, -1, None).get(userName)
          soctRowList.+=((fcsoctApproxAllotmentRow, index))
        }
      }
  ...
  def getEstimatedSIPDate(dayOfMonth:Int): String = {
    DateTimeUtils.getEstimatedSIPDate(dayOfMonth)
  }
  ...
  def getPlacedAtExchangeOrders(omtrfnum: Long): Future[Seq[FcsotRow]] = {
    val query = Fcsot.filter(x => x.sotostmstaterfnum === PLACED_AT_EXCHANGE && x.sotomtrfnum === omtrfnum).forUpdate
    db.run(query.result.transactionally).map(value => {
      value
    })
  }
  ...
  def getOrderPaymentCutOff(subOrderId: Long): Future[Option[FcsoctRow]] = {
    val query = Fcsoct.filter(x => x.soctkey === ORDER_PAY_CUT_OFF_KEY && x.soctsotrfnum === subOrderId)
    db.run(query.result).map(soctRow => {
      soctRow.headOption
    })
  }

  def getSubOrderSiblings(sotrfnum: Long): Future[Seq[FcsotRow]] = {
    val query = for {
      sotRow <- Fcsot.filter(x => x.id === sotrfnum)
      allSotSiblings <- Fcsot.filter(x => x.sotomtrfnum === sotRow.sotomtrfnum)
    } yield (allSotSiblings)

    db.run(query.result)
  }

  def getSubOrderById(sotrfnum: Long): Future[Option[FcsotRow]] = {
    fcsotRepo.getById(sotrfnum)
  }
